...
class FeaturizerTask(Task):
    """Featurizes a single file"""

    in_csv = InputSlot() #input slot for a FoLiA document

    def out_featuretxt(self):
        """Output slot -- outputs a single *.features.txt file"""
        return self.outputfrominput(inputformat='csv', stripextension='.csv', addextension='.features.csv')
